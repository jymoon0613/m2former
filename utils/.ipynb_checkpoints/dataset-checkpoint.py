...
class AIR():
    def __init__(self, root, is_train=True, transform=None, data_len=None):
        self.root = root
        self.is_train = is_train
        self.transform = transform
    ...
    def __getitem__(self, index):
        if self.is_train:
            ...
            img, target = imageio.imread(img_path)[:-20, ...], self.train_label[index]
            if len(img.shape) == 2:
                img = np.stack([img] * 3, 2)
            img = Image.fromarray(img, mode='RGB')
            if self.transform is not None:
            ...
            img, target = imageio.imread(img_path)[:-20, ...], self.test_label[index]
            if len(img.shape) == 2:
                img = np.stack([img] * 3, 2)
                 # img = img[y1:y2, x1:x2, :]
            img = Image.fromarray(img, mode='RGB')
